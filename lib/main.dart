...
const rtcAppId = '2ebd5970fae5404a840a4059c26194ae';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  String _subMenu = 'filters';
  double _colorLevel = 0.5;
  double _filterLevel = 0.5;
  String _filtername = 'ziran2';
  double _redLevel = 0.5;
  double _blurLevel = 6.0;
  int _faceShape = 3;
  ...
  Future<void> _setAppComposer() async {
    await _setComposerCustom(_colorLevel, _filterLevel, _filtername, _redLevel,
        _blurLevel, _faceShape);
  }

  Future<void> _setComposerCustom(
      double colorValue,
      double filterValue,
      String filterName,
      double redLevel,
  ...
  Widget _renderSubMenu(BuildContext context) {
    if (_subMenu == 'filters') {
      return ListView(
        shrinkWrap: true,
        scrollDirection: Axis.horizontal,
    ...
    } else if (_subMenu == 'redlevel') {
      return Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: [
          const Text('Red Level:', textAlign: TextAlign.left),
          Slider(
              value: _redLevel,
              onChanged: _enableComposer
                  ? (double value) async {
    ...
    } else if (_subMenu == 'blurlevel') {
      return Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: [
          const Text('Blur Level:', textAlign: TextAlign.left),
          Slider(
              min: 0.0,
              max: 6.0,
              value: _blurLevel,
    ...
    } else if (_subMenu == 'faceShape') {
      return Row(
        mainAxisAlignment: MainAxisAlignment.spaceAround,
        children: [
          const Text('Face Shape:', textAlign: TextAlign.left),
          Slider(
              min: 0,
              max: 4,
              value: _faceShape.toDouble(),
              onChanged: _enableComposer
                  ? (double value) async {
                      setState(() {
                        _faceShape = value.toInt();
                      });
